...
const CartItem = ({product}) => {
    let {image, price, variations, quantity, id,name ,variants} = product;
        ...
        <SingleCartItem>
            ...
            <CartProInfo>
                <Link href={`/product/${id}`} passHref>
                    <CartProName>{variations?.name}</CartProName>
                </Link>

                {variants?.length > 1 && (
                    <CartProMeta>{variations?.name?variations?.name:name}</CartProMeta>
                )}
            </CartProInfo>

            {/* <CartProPrice>
                {variations?.price?variations?.price:price}
            </CartProPrice> */}
